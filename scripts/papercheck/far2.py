...
def get_clip_auroc_from_features(id_features, ood_features, zeroshot_weights, temperature):
    ...
    for features in [id_features, ood_features]:
        zsw = get_cosine_similarity_matrix_for_normed_features(features, zeroshot_weights, 0.01)
...
def main():
    ...
    temperatures = np.logspace(np.log2(0.01), np.log2(100), num=50, base=2.0)
    for id_name, id_set in HalfTwoDict.items():
